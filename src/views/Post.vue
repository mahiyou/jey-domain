<template>
    ...
    <v-snackbar v-model="errorGetData" class="my-15" multi-line> خطای سرور برای دریافت اطلاعات
        <template v-slot:actions>
            <v-btn color="red" variant="text" @click="errorGetData = false">
                بستن
            </v-btn>
        </template>
    </v-snackbar>
    <v-container v-if="!errorGetData && !loadingGetData" class="post-container">
            ...
            <v-col sm="4" cols="6" class="align">
                <v-row>
                    ...
                    <v-col lg="4" md="5" cols="12" align="left">
                        <v-btn variant="text" density="compact" rounded="pill" color="primary" icon="mdi-pinterest"
                            class="sm-btn d-inline"></v-btn>
                        <v-btn variant="text" density="compact" rounded="pill" color="primary" icon="mdi-twitter"
                            class="sm-btn"></v-btn>
                        <v-btn variant="text" density="compact" rounded="pill" color="primary" icon="mdi-facebook"
                            class="sm-btn"></v-btn>
                    </v-col>
                </v-row>
            </v-col>
        ...
        <div v-if="post.categories.length > 0" class="mt-5">
            <v-btn v-for="index in post.categories.length" :key="index" variant="flat" rounded="pill" color="#e9efff"
                class="ma-1 px-5" href="#">{{ post.categories[index - 1].title }}</v-btn>
        </div>
            ...
            <div v-if="post.comments.length > 0">
                <v-row class="my-10" v-for="comment in post.comments.length" :key="comment">
                    <v-col md="1" cols="2" v-if="comment % 2 != 0" align="left">
                        <v-img v-if="post.comments[comment - 1].author.avatar" class="rounded-circle"
                            :src="post.comments[comment - 1].author.avatar" width="60px"></v-img>
                        <v-img v-if="!post.comments[comment - 1].author.avatar" class="rounded-circle"
                            :src="defaultUserImage" width="60px"></v-img>
                    </v-col>
                    <v-col md="11" cols="10" class="comment-frame" v-if="comment % 2 != 0">
                        <p class="comment-author">{{ post.comments[comment - 1].author.name }}</p>
                        <p class="text-grey-darken-1">{{ new
                            Date(post.comments[comment - 1].date).toLocaleDateString('fa-IR') }}</p>
                        <p>{{ post.comments[comment - 1].content }}</p>
                        <v-btn variant="text" rounded="pill" color="primary" class="float-left"
                            prepend-icon=" mdi-comment-multiple-outline" href="#">پاسخ دادن</v-btn>
                    </v-col>
                    <v-col md="11" cols="10" class="comment-frame" v-if="comment % 2 == 0">
                        <p class="comment-author">{{ post.comments[comment - 1].author.name }}</p>
                        <p class="text-grey-darken-1">{{ new
        ...
        <v-form @submit.prevent="onSubmit" v-model="valid" class="mb-15">
            ...
            <div>آدرس ایمیل شما منتشر نخواهد شد. قسمت های ضروری که با <span class="mr-1 star-color">*</span> مشخص شده اند را
                کامل کنید. </div>
            <div class="mt-8 mb-4">
                <p class="input-title">دیدگاه شما<span class="mr-1 star-color">*</span></p>
                <v-textarea class="my-3" rows="2" v-model="comment" variant="outlined" :rules="commentRules">
                </v-textarea>
            </div>
            <v-row>
                <v-col sm="4" cols=12>
                    <p class="input-title">نام<span class="mr-1 star-color">*</span></p>
                    <v-text-field variant="outlined" class="my-3" v-model="name" :rules="nameRules" required />
                </v-col>
                <v-col sm="4" cols=12>
                    <p class="input-title">ایمیل<span class="mr-1 star-color">*</span></p>
                    <v-text-field variant="outlined" class="my-3" v-model="email" :rules="emailRules" required />
                </v-col>
                <v-col sm="4" cols=12>
                    <p class="input-title">وبسایت</p>
                    <v-text-field variant="outlined" class="my-3" v-model="site" required />
                </v-col>
            </v-row>
            <div class="btn-align">
                <v-btn :loading="loading" variant="flat" color="#4f80ff" rounded="pill" class="submit-btn" height="42px"
                    type="submit" prepend-icon="mdi-check">ارسال نظر
                </v-btn>
            </div>
        </v-form>
...
import { call, IPostInfo } from "@/mocks/API";
